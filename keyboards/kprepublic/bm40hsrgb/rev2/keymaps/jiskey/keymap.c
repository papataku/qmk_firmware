...
#define LAYOUT_ortho_grid LAYOUT_ortho_4x12_1x2uC
...
#define M_EMHL LT(_LOWER,KC_LANG2)      // タップでMacの「英数」とWindowsの「無変換」  ホールドでLowerレイヤー
#define M_KHKR LT(_RAISE,KC_LANG1)      // タップでMacの「かな」とWindowsの「変換」    ホールドでRaiseレイヤー
#define M_ECAJ LT(_ADJUST,KC_ESC)      // タップで「ESC」    ホールドでADJUSTレイヤー
...
[_QWERTY] = LAYOUT_ortho_grid( \
  ...
  KC_LCTL, KC_LGUI, KC_LALT, FUNC2,   M_EMHL,  KC_SPC,           M_KHKR,  TAP_F1,  KC_LEFT, KC_DOWN, KC_RGHT  \
),
...
[_LOWER] = LAYOUT_ortho_grid( \
  ...
  _______, _______, _______, KC_APP,  XXXXXXX, _______,          KC_0,    KC_DOT,  KC_COMM, MA_0X,   _______  \
),
...
[_RAISE] = LAYOUT_ortho_grid( \
  ...
  _______, _______, _______, _______, _______, _______,          XXXXXXX, KC_APP,  _______, _______, _______  \
),
 ...
 * |      |      |      |      |      |      |      |      | PGUP | PGDN |      |      |
 * |------+------+------+------+------+------|------+------+------+------+------+------|
 * |      |      |      |      |      |      |      |      | HOME | END  |CtUp  |MU_BT2|
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |             |      | XXXX |CtLeft|CtDown|CtRigh|
 * `-----------------------------------------------------------------------------------'
 */
[_FUNC1] = LAYOUT_ortho_grid( \
  ...
  _______, _______, _______, _______, _______, _______, _______, _______, KC_HOME, KC_END,  C(KC_UP), KC_BTN2, \
  _______, _______, _______, _______, _______, _______,          _______, XXXXXXX, C(KC_LEFT), C(KC_DOWN), C(KC_RGHT)  \
),
...
[_FUNC2] = LAYOUT_ortho_grid( \
  ...
  _______, _______, _______, XXXXXXX, _______, _______,          KC_0,    KC_DOT,  KC_COMM, MA_0X,   _______ \
),
...
[_10KEY] = LAYOUT_ortho_grid( \
  ...
  _______, _______, _______, _______, EXT_10K, _______,          KC_0,    KC_DOT,  KC_COMM, _______, EXT_10K  \
),
...
[_ADJUST] = LAYOUT_ortho_grid( \
  ...
  _______, DM_RSTP, DM_RSTP, _______, TGL_LOW, _______,          TGL_RIS, _______, KC_MS_L, KC_MS_D, KC_MS_R  \
)
};
...
const rgb_index_segment_t my_rgb_raise_list[] = {{36,12},{255,0}};
const rgb_index_segment_t my_rgb_lower_list[] = {{ 0,12},{255,0}};
const rgb_index_segment_t my_rgb_adjust_list[] = {{0,13}, {23,2}, {35,12},{255,0}};
const rgb_index_segment_t my_rgb_func1_list[] = {{13,10},{255,0}};
const rgb_index_segment_t my_rgb_func2_list[] = {{25,10},{255,0}};
